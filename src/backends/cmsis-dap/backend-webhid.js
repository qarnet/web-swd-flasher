...
import { CmsisDapWebHidTransport } from "./transport-webhid.js";
...
export class CmsisDapWebHidBackend extends ProbeBackend {
  constructor(progressBus, logger = null) {
    super();
    this.transport = new CmsisDapWebHidTransport(logger);
  ...
  async getProbeInfo() {
    ...
    return {
      backend: "cmsis-dap-webhid",
      name: this.transport.device?.productName || "CMSIS-DAP HID",
      manufacturer: this.transport.device?.productName || "Unknown",
      transport: "webhid",
