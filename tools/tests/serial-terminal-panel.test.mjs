...
const FRAGMENT = `<div id="root"><pre id="serial-term-log"></pre><input id="serial-tx-input"><button id="btn-serial-send"></button><button id="btn-serial-clear"></button><button id="btn-serial-download"></button><input id="chk-serial-autoscroll" type="checkbox" checked><input id="chk-serial-cr-newline" type="checkbox" checked><input id="chk-serial-echo" type="checkbox" checked></div>`;
...
test("SerialTerminalPanel writes bytes to terminal buffer", async () => {
  ...
  const panel = new SerialTerminalPanel({ bus, serialManager: new FakeSerialManager() });
...
test("SerialTerminalPanel send via controller", async () => {
  clearHistory();
  makeDom(FRAGMENT);
  const sm = new FakeSerialManager();
  const panel = new SerialTerminalPanel({ bus: new EventBus(), serialManager: sm });
  panel.mount(document.getElementById("root"));
  document.querySelector("#serial-tx-input").value = "hello";
