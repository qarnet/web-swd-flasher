...
| R-003 | Windows driver binding prevents browser interface claim | High | High | High | Zadig WinUSB driver replacement for Pico Debugprobe Interface 0; detect and report claim/open failures with actionable guidance; maintain compatibility matrix by probe firmware/OS | `requestDevice` works but `open`/`claimInterface` fails repeatedly | Project | Mitigating |
